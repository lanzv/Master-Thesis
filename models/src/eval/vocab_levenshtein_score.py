...
def vocab_levenshtein_score(segmented_chants):
    ...
    The score is computed as 1/N sum_{vocab_segment}(normalization_segment_length
    *median(levenshtein distances between vocab  segment and all other vocab segments with same size))
    ...
    levenshtein_score : float
        1/N sum_{vocab_segment}(normalization_segment_length*median(levenshtein 
            distances between vocab  segment and all other vocab segments with same size))
    """
    median_sum = 0
    segment_count = 0
    vocab_sizes = {}
