...
def vocab_levenshtein_score(segmented_chants):
    ...
    The score compute as 1/N sum_{vocab_segment}(max(levenshtein distances with between vocab segment and all rest vocab segments))
    ...
    levenshtein_score : float
        1/N sum_{vocab_segment}(max(levenshtein distances with between vocab segment and all rest vocab segments))
    """
    vocab = set()
    ...
    levenshtein_score = np.average(np.amax(distance_matrix, axis=0))
