...
"""
class Sampler:
    def __init__(self, npylm: "NPYLM", max_word_length: int, max_sentence_length: int):
        ...
        self.p_w_h_cache: list = np.zeros((max_sentence_length+1, max_word_length+1, max_word_length+1, max_word_length+1)) # OffsetArray{Float64}
        # Normalization constants
        self.log_z: list = [0.0 for _ in range(max_sentence_length + 1)] # OffsetVector{Float64}
        ...
        self.scaling_coefficients: list = [0.0 for _ in range(max_sentence_length + 2)] # OffsetVector{Float64}
        ...
        self.viterbi_backward_indices: list = np.zeros((max_sentence_length+1, max_word_length+1, max_word_length+1)) # OffsetArray{Int, 3}
    ...
    def extend_capacity(self, max_word_length: int, max_sentence_length: int):
        # If the existing capacity is already good enough then no need to extend
        if (max_word_length <= self.max_word_length and max_sentence_length <= self.max_sentence_length):
            return
        else:
            self.allocate_capacity(max_word_length, max_sentence_length)

    def allocate_capacity(self, max_word_length: int, max_sentence_length: int):
        ...
        self.log_z = [0.0 for _ in range(max_sentence_length + 1)]
        # Why does this need to be one longer?
        self.scaling_coefficients = [0.0 for _ in range(size + 1)]
        self.viterbi_backward_indices = np.zeros((max_sentence_length + 1, max_word_length + 1, max_word_length + 1))
        ...
        self.p_w_h_cache = np.zeros((max_sentence_length+1, max_word_length+1, max_word_length+1, max_word_length+1))
        # sampler.substring_word_id_cache = Array{Int, 2}(undef, size, max_word_length + 1)
        self.substring_word_id_cache = np.zeros((max_sentence_length+1, max_word_length+1))

    def get_substring_word_id_at_t_k(self, sentence: "Sentence", t: int, k: int):
        """
    ...
    def forward_filtering(self, sentence: "Sentence", with_scaling: bool):
        """
        Performs the forward filtering on the target sentence.
        """
        self.alpha_tensor[0,0,0] = 1.0
        for t in range(1, sentence.length() + 1):
            prod_scaling = 1.0
    ...
    def calculate_alpha_t_k_j(self, sentence: "Sentence", t: int, k: int, j: int, prod_scaling: float):
        ...
        """
        word_k_id = self.get_substring_word_id_at_t_k(sentence, t, k)
        ...
        # Special case 2: This is the case where i == 0 but j != 0, i.e. the first gram is BOS (but the second gram is a normal word)
            ...
            word_j_id = self.get_substring_word_id_at_t_k(sentence, t - k, j)
            self.word_ids[0] = BOS
            ...
            for i in range(1, min(t - k - j, self.max_word_length)+1):
                word_i_id = self.get_substring_word_id_at_t_k(sentence, t - k - j, i)
                word_j_id = self.get_substring_word_id_at_t_k(sentence, t - k, j)
                # The first gram
    ...
    def backward_sampling(self, sentence: "Sentence"):
        """
        ...
        j = [0]
        sum_length = 0
        self.backward_sample_k_and_j(sentence, t, 1, k, j)
        ...
        while (t > 0):
            ...
            if t == 1:
                ...
                self.backward_sample_k_and_j(sentence, t, next_word_length, k, j)
            segment_lengths.append(k[0])
            t -= k[0]
            if j[0] == 0:
                # println("t is $(t)")
    ...
    def backward_sample_k_and_j(self, sentence: "Sentence", t: int, third_gram_length: int, sampled_k: list, sampled_j: list):
        """
        ...
        for k in range(1, min(t, self.max_word_length) + 1):
            for j in range(1, min(t - k, self.max_word_length)+1):
                ...
                if t < sentence.length():
                    assert(t + third_gram_length <= sentence.length())
                    ...
                    word_t_id = self.get_substring_word_id_at_t_k(sentence, t + third_gram_length, third_gram_length)
    ...
    def blocked_gibbs_segment(self, sentence: "Sentence", with_scaling: bool):
        """
        Does the segment part in the blocked Gibbs algorithm (line 6 of Figure 3 of the paper)
        """
        for i in range(0, sentence.length() + 1):
    ...
    def viterbi_argmax_calculate_alpha_t_k_j(self, sentence: "Sentence", t: int, k: int, j:int):
    ...
    def viterbi_forward_filtering(self, sentence: "Sentence"):
        for t in range(1, sentence.length() + 1):
            for k in range(1, min(t, self.max_word_length) + 1):
    ...
    def viterbi_argmax_backward_sample_k_and_j_to_eos(self, sentence: "Sentence", t:int, third_gram_length: int, argmax_k: list, argmax_j: list):
        """
    ...
    def viterbi_backward_sampling(self, sentence: "Sentence"):
        segment_lengths = []
        t = sentence.length()
        sum_length = 0
        kc = [0]
        jc = [0]
        self.viterbi_argmax_backward_sample_k_and_j_to_eos(sentence, t, 1, kc, jc)
        k = kc[0]
        j = jc[0]
        assert(k <= self.max_word_length)
        # I'm not sure yet why the segments array should contain their lengths instead of the word ids themselves. I guess it's just a way to increase efficiency and all that.
    ...
    def viterbi_decode(self, sentence: "Sentence"):
        """
        ...
        self.log_z[0] = 0.0
        for t in range(0, sentence.length()+1):
    ...
    def compute_log_forward_probability(self, sentence: "Sentence", with_scaling: bool):
        """
    ...
    def enumerate_forward_variables(self, sentence: "Sentence", with_scaling: bool):
        # array_length = length(sentence) + 1
        for i in range(0, sentence.length() + 1):
            for j in range(0, self.max_word_length + 1):
                self.substring_word_id_cache[i,j] = 0
