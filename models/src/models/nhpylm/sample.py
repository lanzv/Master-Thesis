...
"""
class Sampler:
    def __init__(self, npylm: "NPYLM", min_word_length: int, max_word_length: int, max_chant_length: int, n_gram: int = 3):
        self.n_gram = n_gram
        self.npylm: "NPYLM" = npylm
        # The word_ids of the current 3-gram being calculated
        self.word_ids: list = [0 for _ in range(self.n_gram)]
        ...
        if n_gram == 3:
            ...
            self.alpha_tensor: list = np.zeros(((max_chant_length+1, max_word_length+1)))
            self.p_w_h_cache: list = np.zeros((max_chant_length+1, max_word_length+1, max_word_length+1))
            self.viterbi_backward_indices: list = np.zeros((max_chant_length+1, max_word_length+1))
        else:
            raise Exception("We don't support ngram {}".format(n_gram))
        ...
        if (not self.n_gram == 2) and (min_word_length != 1):
            raise Exception("We don't support min_word_length in other n_grams than n_gram == 2")
    ...
    def allocate_capacity(self, max_word_length: int, max_chant_length: int):
        ...
        if self.n_gram == 3:
            ...
            # It needs to be 1 longer than length, because we have to accomodate for the index 0, which indicates that we have BOS as one of the grams.
            self.alpha_tensor = np.zeros((size+1, max_word_length+1))
            # sampler.p_w_h_cache = Array{Float64, 4}(undef, size, max_word_length + 1, max_word_length + 1, max_word_length + 1)
            self.p_w_h_cache = np.zeros((max_chant_length+1, max_word_length+1, max_word_length+1))
            self.viterbi_backward_indices = np.zeros((max_chant_length + 1, max_word_length + 1))
        else:
            raise Exception("We don't support ngram {}".format(self.n_gram))
    ...
    def blocked_gibbs_segment(self, chant: "Chant", with_scaling: bool):
        ...
        if self.n_gram == 3:
            return self.backward_sampling_trigram(chant)
        elif self.n_gram == 2:
            return self.backward_sampling_bigram(chant)
        else:
            raise Exception("We don't support ngram {}".format(self.n_gram))
    ...
    def forward_filtering(self, chant: "Chant", with_scaling: bool):
        ...
        if self.n_gram == 3:
            ...
            for t in range(1, chant.length() + 1):
                ...
                for k in range(1, min(t, self.max_word_length) + 1):
                    ...
                    for j in range(0 if t == k else 1,  min(t-k, self.max_word_length) + 1):
                        ...
                        self.calculate_alpha_t_k_j_trigram(chant, t, k, j, prod_scaling)
            ...
            for t in range(self.min_word_length, chant.length() + 1):
                prod_scaling = 1.0
                for k in range(self.min_word_length, min(t, self.max_word_length) + 1):
                    if (with_scaling and k > self.min_word_length and t-k > self.min_word_length):
                        prod_scaling *= self.scaling_coefficients[t - k + 1]
                    self.alpha_tensor[t,k] = 0
                    self.calculate_alpha_t_k_bigram(chant, t, k, prod_scaling)
                # Perform scaling operations on the alpha tensor, in order to avoid underflowing.
                if (with_scaling):
                    sum_alpha = 0.0
                    for k in range(self.min_word_length, min(t, self.max_word_length)+1):
                        sum_alpha += self.alpha_tensor[t,k]
                    if not sum_alpha > 0.0:
                        raise Exception("sum_alpha <= 0.0")
                    self.scaling_coefficients[t] = 1.0 / sum_alpha
                    for k in range(self.min_word_length, min(t, self.max_word_length) + 1):
                        self.alpha_tensor[t,k] *= self.scaling_coefficients[t]
        else:
            raise Exception("We don't support ngram {}".format(self.n_gram))
    ...
    def calculate_alpha_t_k_bigram(self, chant: "Chant", t: int, k: int, prod_scaling: float):
        ...
        # Speical case t-k == 0 means there's no actual *second* gram, i.e. the first token is BOS!
        if t-k == 0:
            self.word_ids[0] = BOS
            self.word_ids[1] = word_k_id
            # Probably of the word with length k, which is the last (3rd) word.
            p_w_h = self.npylm.compute_p_w_of_nth_word_chars(chant_as_chars, self.word_ids, 1, t - k, t - 1)
            if not p_w_h > 0.0:
                raise Exception("p_w_h <= 0.0")
            # In this case, the expression becomes the following.
            self.alpha_tensor[t,k] = p_w_h *prod_scaling # * self.alpha_tensor[t - k,0]
            # The last index here is i. i == 0.
            self.p_w_h_cache[t,k,0] = p_w_h
            return
        elif t-k < self.min_word_length:
            return
        else:
            # Perform the normal marginalization procedure in all other cases
            sum = 0.0
            for j in range(self.min_word_length, min(t - k, self.max_word_length)+1):
                if (not j + k == t) and t - k - j < self.min_word_length:
                    continue
                ...
                self.p_w_h_cache[t,k] = p_w_h

                temp = p_w_h * self.alpha_tensor[t - k,j]
                sum += temp
            if not sum > 0.0:
                raise Exception("sum <= 0.0")
            self.alpha_tensor[t,k] = sum * prod_scaling

    def backward_sampling_bigram(self, chant: "Chant"):
        """
        ...
        k = [0]
        sum_length = 0
        self.backward_sample_k(chant = chant, t = t, previous_word_length = 1, sampled_k=k)
        ...
        if not (k[0] > 0):
            raise Exception("k[0] <= 0")

        # Record the second-to-last word we just sampled.
        t -= k[0]
        sum_length += k[0]
        next_word_length = k[0]

        while (t > 0):
            self.backward_sample_k(chant = chant, t = t, previous_word_length = next_word_length, sampled_k = k)
            segment_lengths.append(k[0])
            t -= k[0]
            sum_length += k[0]
            next_word_length = k[0]
    ...
    def backward_sample_k(self, chant: "Chant", t: int, previous_word_length: int, sampled_k: list):
        """
        Returns k, which denote the offsets for word boundaries of the word we are interested in sampling.
        "next_word" really means the target word, the last gram in the bigram, e.g. the EOS in p(EOS | c^N_{N-k})
        """
        table_index = 0
        chant_as_chars = chant.characters
        sum_p = 0.0
        for k in range(self.min_word_length, min(t, self.max_word_length) + 1):
            ...
            word_t_id = EOS
            if t < chant.length():
                if not (t + previous_word_length <= chant.length()):
                    raise Exception("t + third_gram_length >= chant.length()")
                if not (previous_word_length > 0):
                ...
                word_t_id = self.get_substring_word_id_at_t_k(chant, t + previous_word_length, previous_word_length)
            self.word_ids[0] = word_k_id
            self.word_ids[1] = word_t_id
            p_w_h = 0.0
            if t == chant.length():
                # The only exception to caching is the situation where the last gram is EOS.
                # p(EOS | c^N_{N-k})
                p_w_h = self.npylm.compute_p_w_of_nth_word_chars(chant_as_chars, self.word_ids, 1, t, t)
            else:
                # In all other scenarios, we should have already cached this value.
                p_w_h = self.p_w_h_cache[t + previous_word_length, previous_word_length, k]

            if not (self.alpha_tensor[t,k] > 0):
                raise Exception("self.alpha_tensor[t, k] <= 0")
            # p(2rd_gram | c^N_{N-k}) * α[N][k])
            p = p_w_h * self.alpha_tensor[t,k]
            if not (p > 0):
                raise Exception("p <= 0")
        ...
        if not (table_index <= self.max_word_length):
            raise Exception("table_index > self.max_word_length * self.max_word_length")

        # Eventually, the table should have (min(t, sampler.max_word_length)) + 1 entries
    ...
    def calculate_alpha_t_k_j_trigram(self, chant: "Chant", t: int, k: int, j: int, prod_scaling: float):
    ...
    def backward_sampling_trigram(self, chant: "Chant"):
        """
        Performs the backward sampling on the target chant.
        """
        t = chant.length()
    ...
    def viterbi_decode(self, chant: "Chant"):
        ...
        if self.n_gram == 3:
            return self.viterbi_backward_sampling_trigram(chant)
        elif self.n_gram == 2:
            return self.viterbi_backward_sampling_bigram(chant)
        else:
            raise Exception("We don't support ngram {}".format(self.n_gram))
        ...
        if self.n_gram == 3:
            for t in range(1, chant.length() + 1):
                for k in range(1, min(t, self.max_word_length) + 1):
                    ...
                    if t == k:
                        self.viterbi_argmax_calculate_alpha_t_k_j_trigram(chant, t, k, 0)
                    # Note that in the t==k case, we will have range 1:0 which is automatically empty, so the following code will not be run.
                    for j in range(1, min(t - k, self.max_word_length)+1):
                        self.viterbi_argmax_calculate_alpha_t_k_j_trigram(chant, t, k, j)
        elif self.n_gram == 2:
            for t in range(self.min_word_length, chant.length() + 1):
                for k in range(self.min_word_length, min(t, self.max_word_length) + 1):
                    self.viterbi_argmax_calculate_alpha_t_k_bigram(chant, t, k)
        else:
            raise Exception("We don't support ngram {}".format(self.n_gram))
    ...
    def viterbi_argmax_calculate_alpha_t_k_bigram(self, chant: "Chant", t: int, k: int):
        word_k_id = self.get_substring_word_id_at_t_k(chant, t, k)
        chant_as_chars = chant.characters
        if not (t <= self.max_chant_length + 1):
            raise Exception("t > self.max_chant_length+1")
        if not (k <= self.max_word_length):
            raise Exception("k > self.max_word_length")
        # Special case 1: t-k == 0 means there's no actual *second* gram, i.e. the first token is BOS!
        if t-k == 0:
            self.word_ids[0] = BOS
            self.word_ids[1] = word_k_id
            # Compute the probability of this word with length k
            ...
            # I think the scaling is to make sure that this thing doesn't underflow.
            # Store the values in the cache
            self.alpha_tensor[t,k] = np.log(p_w_h)
            # Here is the difference.
            self.viterbi_backward_indices[t,0] = 0
            return
        elif t-k < self.min_word_length:
            self.viterbi_backward_indices[t,k] = 0
            return
        else:
            # Perform the normal marginalization procedure in all other cases
            max_log_p = 0.0
            ...
            self.alpha_tensor[t,k] = max_log_p
            # We use the viterbi_backward_indices matrix to store the i value that maximizes the possibility of the trigram.
            self.viterbi_backward_indices[t,k] = argmax



    def viterbi_backward_sampling_bigram(self, chant: "Chant"):
        segment_lengths = []
        t = chant.length()
        sum_length = 0
        if (t < self.min_word_length) or (t >= self.min_word_length and t <= 2*self.min_word_length):
            segment_lengths.append(t)
            segment_lengths.reverse()
            return segment_lengths
        kc = [0]
        self.viterbi_argmax_backward_sample_k_to_eos(chant, t, kc)
        k = kc[0]
        if not (k <= self.max_word_length):
            raise Exception("k > self.max_word_length")
        # I'm not sure yet why the segments array should contain their lengths instead of the word ids themselves. I guess it's just a way to increase efficiency and all that.
        ...
        if not (k > 0):
            raise Exception("k <= 0")
        # We knew that j is the index that maximizes the possibility of the bigram
        j = self.viterbi_backward_indices[int(t),int(k)]
        if not j >= 0:
            raise Exception("j < 0")
        if not j <= self.max_word_length:
            raise Exception("j > self.max_word_length")
        ...
        if j == 0:
            chant.show()
            print(t, k, j, sum_length, chant.length())
            if not sum_length == chant.length():
                raise Exception("sum_length != chant.length()")
            segment_lengths.reverse()
            return segment_lengths
    ...
    def viterbi_argmax_backward_sample_k_to_eos(self, chant: "Chant", t:int, argmax_k: list):
        """
        ...
        for k in range(self.min_word_length, min(t, self.max_word_length) + 1):
            ...
            if not self.alpha_tensor[t,k] <= 0:
                raise Exception("self.alpha_tensor[t,k] > 0")
            temp = np.log(p_w_h) + self.alpha_tensor[t,k]
            if not temp <= 0:
                raise Exception("temp > 0")
    ...
    def viterbi_argmax_calculate_alpha_t_k_j_trigram(self, chant: "Chant", t: int, k: int, j:int):
    ...
    def viterbi_backward_sampling_trigram(self, chant: "Chant"):
        segment_lengths = []
        t = chant.length()
        sum_length = 0
        kc = [0]
    ...
    def compute_log_forward_probability(self, chant: "Chant", with_scaling: bool):
        ...
        if not with_scaling:
            ...
            if self.n_gram == 3:
                ...
                for k in range(1, min(t, self.max_word_length)+1):
                    if not (self.alpha_tensor[t,k] > 0.0):
                        raise Exception("self.alpha_tensor[t,k] <= 0.0")
                    alpha_eos += self.alpha_tensor[t,k]
                if not (alpha_eos > 0.0):
                    raise Exception("alpha_eos <= 0.0")
                return np.log(alpha_eos)
            else:
    ...
    def enumerate_forward_variables(self, chant: "Chant", with_scaling: bool):
        ...
        if self.n_gram == 3:
            ...
            for j in range(1, min(t - k, self.max_word_length) + 1):
                prob_sum = 0.0
                self.word_ids[0] = self.get_substring_word_id_at_t_k(chant, t - k, j)
                self.word_ids[1] = EOS
                p_w_h = self.npylm.compute_p_w_of_nth_word_chars(chant_as_chars, self.word_ids, 1, t, t)
                if not (p_w_h > 0.0):
                    raise Exception("p_w_h <= 0.0")
                prob_sum += p_w_h * self.alpha_tensor[t-k,j]
                self.alpha_tensor[t,k] = prob_sum
                alpha_eos += prob_sum
            if with_scaling:
                self.scaling_coefficients[t] = 1.0 / alpha_eos
        else:
